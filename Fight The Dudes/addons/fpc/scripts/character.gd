...
func _process(_delta):
	...
	$myUI/Label4.text = "/"+str(Global.enemisForScene)
	$myUI/Label3.text = str(Global.enemiesTS)
	
	mouse_sensitivity = Global.sensitivity;
	#if Input.is_action_just_pressed('sprint'):
		#sprintToggle = not sprintToggle;
		#if sprintToggle:
			#$myUI/sprint.modulate = Color.GRAY;
		#else:
			#$myUI/sprint.modulate = Color.WHITE;
	#
	health = Global.health;
	$myUI/Label.text = str(health);
	...
	if len(Global.bodies) >= 1 and Global.bodies[0] != null and Global.bodies[0].is_in_group("interact"):
		$Reticle/dot.modulate = Color("#ea302f");
	else:
		$Reticle/dot.modulate = Color("#fff");
...
func _physics_process(delta):
	...
	if state == "sprinting" and not Global.ui:
		$AnimationPlayer.play("running");
	elif inp_direction.x != 0 and not Global.ui:
		$AnimationPlayer.play("walking");
	else:
		if not Global.ui:
			$AnimationPlayer.play("RESET");
			$AnimationPlayer.stop()
	if $Head/view.is_colliding():
		Global.bodies = [];
		Global.bodies.append($Head/view.get_collider());
	else:
		Global.bodies = [];
	
	if not $Head/view.is_colliding():
		Global.bodies.resize(0);
	...
	if not immobile: # Immobility works by interrupting user input, so other forces can still be applied to the player
		...
		#input_dir = Input.get_vector(-direction.x,direction.x,-direction.y,direction.y)
	...
	handle_state(inp_direction)
	if dynamic_fov: # This may be changed to an AnimationPlayer
		update_camera_fov()

	if view_bobbing:
		play_headbob_animation(inp_direction)
...
func handle_movement(delta, input_dir):
	var direction = inp_direction.rotated(-HEAD.rotation.y) * delta
	direction = HEAD.transform.basis * Vector3(inp_direction.x, 0, inp_direction.y)
	if not Global.ui:
		move_and_slide()
...
func handle_head_rotation():
	if invert_camera_x_axis:
		...
		$Mesh.rotation_degrees.y -= (mouseInput.x * mouse_sensitivity * -1)
	else:
		HEAD.rotation_degrees.y -= (mouseInput.x * mouse_sensitivity)
		$Mesh.rotation_degrees.y -= (mouseInput.x * mouse_sensitivity)
...
func _unhandled_input(event : InputEvent):
	if event is InputEventMouseMotion and Input.mouse_mode == Input.MOUSE_MODE_CAPTURED || event is InputEventScreenDrag:
